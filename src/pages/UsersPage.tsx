...
export function UsersPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    store.addUser(formData as any);
    toast.success("بەکارهێنەر زیاد کرا بە سەرکەوتوویی");
    ...
    <div className="animate-in fade-in duration-700 pb-10">
      ...
      {printMode === "ALL_BADGES" && (
        <div className="hidden print:block print:w-full" dir="rtl">
          <div className="grid grid-cols-4 gap-[2mm] content-start">
             {filteredUsers.map((user) => (
               <div
                 key={user.id}
      ...
      {selectedUser && (
        ...
        >
          ...
          >
            ...
            <div className="w-full md:w-2/5 bg-gradient-to-br from-indigo-700 to-indigo-900 p-10 text-white flex flex-col items-center justify-center text-center relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-[80px] -mr-20 -mt-20 pointer-events-none"></div>
              <div className="absolute bottom-0 left-0 w-48 h-48 bg-indigo-500/20 rounded-full blur-[60px] -ml-20 -mb-20 pointer-events-none"></div>

              <div className="w-28 h-28 bg-white rounded-[2.5rem] flex items-center justify-center text-indigo-700 font-black text-5xl mb-6 shadow-2xl border-4 border-indigo-400/20 relative z-10 transition-transform hover:scale-105">
              ...
              <span className="px-4 py-1.5 bg-indigo-900/50 backdrop-blur-md rounded-xl text-xs font-black mb-8 uppercase tracking-widest relative z-10 shadow-inner border border-indigo-500/30 text-indigo-100">
                {getCategoryLabel(selectedUser.category)}
              </span>
              ...
              <div className="bg-white p-4 rounded-[2rem] shadow-2xl relative group mb-8 border border-white/10 transform transition-all hover:scale-105">
                ...
                >
                  <QrCode className="w-12 h-12 text-indigo-600 drop-shadow-md mb-2" />
              ...
              <button
                onClick={() => handlePrint("BADGE")}
                className="w-full max-w-[200px] flex items-center justify-center gap-2 bg-white hover:bg-indigo-50 text-indigo-700 px-6 py-4 rounded-2xl font-black transition-all shadow-xl active:scale-95 z-10"
              >
